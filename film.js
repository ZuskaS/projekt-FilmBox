...
noteFormElement.addEventListener('submit', (event) => {
  ...
  const termsCheckboxElement = document.querySelector('#terms-checkbox');
  const message = messageInputElement.value;
  alert(`Zpráva: ${message}`);
  if (message === '') {
    messageInputElement.classList.add('is-invalid');
    messageInputElement.focus();
    return;
  }
  if (!termsCheckboxElement.checked) {
    termsCheckboxElement.classList.add('is-invalid');
    termsCheckboxElement.focus();
    return;
  }
  noteFormElement.innerHTML = `<p class="card-text">${message}</p>`;
});
